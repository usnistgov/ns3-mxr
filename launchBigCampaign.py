...
rclAmEnabled = ['true']
rfhAppId = [1,2,3,4,5,6,7]
# rfhAppId = [1,4,6,650]
rfhTraffic = ['true']
# txPower = [i for i in range(0, 21, 10)]
txPower = [0,10,20]
beamformingIntervals = [2]
...
RngRunCount = 2  # There are 8 runs from 3 to 10
...
def create_folder_structure(codebook_file, beamforming_interval):
    ...
    folder_path = os.path.join("Results", codebook_file, folder_name)
...
def start_simulation(params):
    ...
    folder_path = os.path.join("Results", codebookFile, folder_name)
    ...
    os.system(f'./ns3 run "CDFSurgeryNorm --rlcAmEnabled={rclAmEnabled} --rfhTraffic={rfhTraffic} --rfhAppId={rfhAppId} --txPower={txPower} --scenario={scenario} --runNumber={rngRun} --codebookFile={codebookFile} --beamformingInterval={beamformingInterval}" > {output_file} 2>&1')
