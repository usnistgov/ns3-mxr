...
//    std::cout << "TX ADDED\n";
    //     std::cout << Ipv4Address::ConvertFrom (localAddrs)
    //   << "->"
    //   << InetSocketAddress::ConvertFrom (dstAddrs).GetIpv4 ()
    //   << "("
    //   << seqTsSizeHeader.GetSeq ()
    //   << ")\n";
    //   std::cout << "END TX ADDED\n";
...
//   g_rxPacketsForDelayCalc.insert (std::pair < std::string, PacketWithRxTimestamp> (mapKey, mapValue));
  g_rxPacketsForDelayCalc.emplace(mapKey, mapValue);
    ...
    // std::cout << "bytes:" << totalBytesSent << std::endl;
}
                       ...
                       const Address &dstAddrs, const SeqTsSizeHeader &seqTsSizeHeader)
{
  ...
  if (range.first == range.second) {
    ...
    // To display all the string keys of the map:
    // for (const auto& kv : g_rxPacketsForDelayCalc) {
    //   std::cout << kv.first << std::endl;
    // }
    if (seqTsSizeHeader.GetSeq()!=0){
...
main(int argc, char* argv[])
{
    ...
    mmwaveHelper->SetUeBeamformingCodebookAttribute(
        "CodebookFilename",
        StringValue("../../src/mmwave/model/Codebooks/1x16.txt"));
    // 2. set the antenna dimensions
    mmwaveHelper->SetUePhasedArrayModelAttribute("NumRows", UintegerValue(1));
    mmwaveHelper->SetUePhasedArrayModelAttribute("NumColumns", UintegerValue(16));
    ...
    //     asciiTraceHelper.CreateFileStream(GenerateFileName("snr", blockageFDA, distribution,txPowerStr));
    
    AsciiTraceHelper asciiTraceHelper;
    Ptr<OutputStreamWrapper> snrStreamWrapper =
    asciiTraceHelper.CreateFileStream(GenerateFileName(scenario, "snr", rlcAmEnabled, distribution, txPowerStr,std::to_string(runNumber)));
    ...
    mmWaveUePhy->TraceConnectWithoutContext(
     ...
     Ptr<OutputStreamWrapper> ueStats = 
        asciiTraceHelper.CreateFileStream(GenerateFileName(scenario, "UE", rlcAmEnabled, distribution, txPowerStr,std::to_string(runNumber)));
        ...
        *ueStats->GetStream() << "TIME,MCS,RV,SINR,TBLER,Corrupt" << std::endl;
    ...
    // mmWaveUePhy->TraceConnectWithoutContext("ReportCurrentCellRsrpSinr",
    // MakeCallback(&LogReceivedPacketSNR,snrFile));
    // mmWaveUePhy->TraceConnectWithoutContext("ReportCurrentCellRsrpSinr",
    // MakeBoundCallback(&LogReceivedPacketSNR, std::ref(snrFile)));
    ...
    // The APP is from the UE to the remote host so UL
...
// }
    ...
    // Ptr<OutputStreamWrapper> throughputStreamWrapper = asciiTraceHelperThroughput.CreateFileStream(
    //     GenerateFileName("throughput", blockageFDA, distribution,txPowerStr));

    Ptr<OutputStreamWrapper> throughputStreamWrapper =
    asciiTraceHelper.CreateFileStream(GenerateFileName(scenario, "throughput", rlcAmEnabled, distribution,txPowerStr,std::to_string(runNumber)));    
    ...
    //     ascii.CreateFileStream(GenerateFileName("distribution",distribution,txPowerStr));
        Ptr<OutputStreamWrapper> packetOutputStream =
    ...
    *packetOutputStream->GetStream() << "time(sec)\ttx/rx\tPktSize(bytes)" << std::endl;
    ...
    Ptr<OutputStreamWrapper> delayTraceStream = ascii.CreateFileStream((GenerateFileName(scenario, "delay", rlcAmEnabled, distribution, txPowerStr,std::to_string(runNumber))));
    *delayTraceStream->GetStream() << "time(s)\trxNodeId\tsrcIp\tdstIp\tseqNum\tdelay(ms)" << std::endl;
